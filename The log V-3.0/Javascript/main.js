...
function nextCall() {
  Alert.render(
    'Chose Area Of Effect! <br /> <button class="valuez" onclick="swin.push(\'first\'); Alert.ok(); cliker_check++; Affect();">First Module</button> <button class="valuez" onclick="swin.push(\'second\'); Alert.ok(); cliker_check++; Affect();" id="second">Second Module</button> <button class="valuez" onclick="swin.push(\'both\'); cliker_check++; Alert.ok(); Affect()" id="both">Both Modules</button> <button class="valuez" onclick="Alert.ok(); swin.push(\'none\'); Affect()">None</button>'
  );
  if (ping_Pong == 0) {
